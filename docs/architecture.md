...
flowchart TB
    subgraph ING["Offline indexing (one-time)"]
        ...
        B2 --> C
        C --> D[embedder<br/>Gemini text-embedding-004<br/>+ disk cache]
    ...
    subgraph QUERY["Query time (per request)"]
        ...
        PT --> LLM[ChatLLM<br/>gemini-1.5-flash]
...
| `src/embedder.py`       | Gemini embeddings + disk cache + L2 norm                  | B       |
...
| `src/llm.py`            | Thin Gemini chat wrapper                                  | D       |
...
The LLM (`gemini-1.5-flash`, temperature 0.2) returns the answer. Every stage writes a structured JSON line to `logs/pipeline.log` so a reviewer can see the retrieved docs, their scores, and the exact prompt that was sent to the LLM.
...
5. **No framework lock-in, low operational cost.** The vector store is a single `.npy` file; the metadata is `.jsonl`. Streamlit Cloud can cold-start the whole stack from a GitHub clone, load `embeddings.npy` into RAM, and serve queries with one Gemini API call per question. Because Gemini's free tier covers both chat and embeddings, the entire system runs at zero marginal cost for grading. For a 60-mark academic project the operational complexity is negligible and there's no vendor coupling — swapping back to OpenAI would require changing only `src/embedder.py` and `src/llm.py`.
...
| Embedding API rate limits                            | `_embed_batch` retries with backoff; disk cache avoids re-embedding anything.     |
...
ope the boost to similar clusters.
- BM25 is rebuilt in memory at startup — fine for a 50k-chunk corpus, not for millions.
- The current refusal threshold is a single global constant (`0.15`); ideally it would be calibrated per source (CSV scores typically run lower than PDF scores because CSV rows are short).
